...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            <h2 class="text-2xl font-bold">Why Us Section</h2>
            <p class="text-gray-500">Kelola keunggulan dan alasan memilih layanan</p>
        </div>
        <a href="{{ route('admin.landing.whyus.create') }}" class="bg-primary text-white px-4 py-2 rounded-lg hover:bg-primary/90 transition-colors inline-flex items-center gap-2">
            <i class="ri-add-line"></i> Tambah Why Us
        </a>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($whyUsItems as $index => $item)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                            <a href="{{ route('admin.landing.whyus.edit', $item->id) }}" class="bg-primary text-white px-3 py-1 rounded hover:bg-primary/90 transition-colors inline-flex items-center gap-1">
                                <i class="ri-edit-line text-xs"></i> Edit
                            </a>
                            <form action="{{ route('admin.landing.whyus.destroy', $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus?')">
                                @csrf
                                @method('DELETE')
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-8 text-center text-gray-500">
                            <div class="flex flex-col items-center">
                                ...
                                <p>Belum ada data why us section</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
